      ...
      {(data || !loading) && (
        <>
          ...
          <section id="leaderboard-section" className="baitotrack-leaderboard-root" aria-label="Leaderboard">
            ...
            {/* Header Overlay (Dynamic text on top-left) */}
            <div className="leaderboard-header-overlay">
              <div className="title-row">
                <span className="trophy-emoji">🏆</span>
                <h2 className="leaderboard-title-text">Leaderboard</h2>
              </div>
              <p className="leaderboard-subtitle-text">Top promoters who shine every day! ✨</p>
            </div>

            {/* TOP 3 PODIUM OVERLAYS */}
            {/* Winner 2 (Silver - Left) */}
            <div className="podium-avatar-overlay silver-avatar">
              {renderAvatar(2)}
            </div>
            <div className="podium-text-overlay silver-text">
              <div className="overlay-promoter-name">{entries.length > 1 ? entries[1].promoter_name : "Alex"}</div>
              <div className="overlay-signup-count">{entries.length > 1 ? entries[1].valid_count : 321}</div>
              <div className="overlay-signup-label">signups</div>
            </div>

            {/* Winner 1 (Gold - Center) */}
            <div className="podium-avatar-overlay gold-avatar">
              {renderAvatar(1)}
            </div>
            <div className="podium-text-overlay gold-text">
              <div className="overlay-promoter-name">{entries.length > 0 ? entries[0].promoter_name : "Jessica"}</div>
              <div className="overlay-signup-count">{entries.length > 0 ? entries[0].valid_count : 387}</div>
              <div className="overlay-signup-label">signups</div>
            </div>

            {/* Winner 3 (Bronze - Right) */}
            <div className="podium-avatar-overlay bronze-avatar">
              {renderAvatar(3)}
            </div>
            <div className="podium-text-overlay bronze-text">
              <div className="overlay-promoter-name">{entries.length > 2 ? entries[2].promoter_name : "Samantha"}</div>
              <div className="overlay-signup-count">{entries.length > 2 ? entries[2].valid_count : 278}</div>
              <div className="overlay-signup-label">signups</div>
            </div>

            {/* RIGHT SIDE TABLE OVERLAY (Rankings 4+) */}
            <div className="rankings-table-overlay">
              <div className="table-wrapper spec-table-wrapper-overlay">
                <table className="custom-table clean-table overlay-table">
                  <thead>
                    <tr>
                    ...
                    {entries.slice(3).map((entry, idx) => {
                      ...
                      return (
                        <tr key={entry.rank}>
                          ...
                          <td>
                            <div className="name-avatar-cell-overlay">
                              <div className="mini-avatar-overlay">
                                {renderAvatar(entry.rank)}
                              </div>
                              <span className="promoter-name-text-overlay">{entry.promoter_name}</span>
                            </div>
                          </td>
                          <td className="progress-value-cell-overlay">
                            <div className="bar-val-wrapper-overlay">
                              <div className="rank-progress-bg-overlay">
                                <div
                                  className="rank-progress-fill-overlay"
                                  style={{ width: `${progressPercent}%` }}
                                />
                              </div>
                              <span className="valid-count-number-overlay">
                                {entry.valid_count}
                              </span>
                            </div>
                          </td>
